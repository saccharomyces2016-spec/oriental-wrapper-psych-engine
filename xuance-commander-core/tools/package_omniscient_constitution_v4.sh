#!/bin/bash
# ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4 任務包打包腳本
# 審核日期：2026-01-13
...
PACKAGE_NAME="ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_PACKAGE_${TIMESTAMP}"
OUTPUT_DIR="tmp/${PACKAGE_NAME}"
ZIP_NAME="tmp/${PACKAGE_NAME}.zip"

mkdir -p "$OUTPUT_DIR"
echo "📦 開始打包 ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4 任務包..."

# 1. 審核報告和追問包
cp docs/ops/analysis/ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_AUDIT.md "$OUTPUT_DIR/"
cp docs/task_packets/advisor/ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_QUESTIONS.md "$OUTPUT_DIR/"

# 2. 原始文件（GPT V4 處理後版本）
cp docs/ops/analysis/ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL_V4.md "$OUTPUT_DIR/ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md"
...
# 14. 之前的審核報告（作為參考）
cp docs/ops/analysis/ENGINE_CORE_OMNISCIENT_CONSTITUTION_V3_AUDIT.md "$OUTPUT_DIR/" 2>/dev/null || echo "⚠️  V3_AUDIT 不存在，跳過"
cp docs/task_packets/advisor/ENGINE_CORE_OMNISCIENT_CONSTITUTION_V3_QUESTIONS.md "$OUTPUT_DIR/" 2>/dev/null || echo "⚠️  V3_QUESTIONS 不存在，跳過"

# 15. 建立 README
cat > "$OUTPUT_DIR/README.md" << 'EOF'
# ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4 任務包

**建立日期**：2026-01-13  
**狀態**：OPEN（等待裁示）  
**目標**：審核 `ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md`（GPT V4 處理後版本），確認是否符合任務需求
...
#### 審核報告
- `ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_AUDIT.md` - 審核報告 ⭐⭐⭐

#### 追問包
- `ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_QUESTIONS.md` - 追問包 ⭐⭐⭐

#### 原始文件
- `ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md` - GPT V4 處理後的原始文件
...
1. ⭐⭐⭐ **缺失 Priors 的 Rigidity 預設值**：應該是 0 還是 0.50？
   ...
   - DIRECTIVE REV.B：0.0（實際內容）
   - CONSTITUTION Section 7.2：0.50
   - engine/score_facet.py：0.5（預設參數）
   - GPT V4 聲稱：最終裁示為 0.50（MIXED default），與 REV.B 一致（但實際檢查結果不一致）
...
echo ""
echo "🎯 關鍵文件："
echo "   - ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_AUDIT.md"
echo "   - ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4_QUESTIONS.md"
echo "   - ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md"
echo "   - engine/score_facet.py（V4 實作）"
