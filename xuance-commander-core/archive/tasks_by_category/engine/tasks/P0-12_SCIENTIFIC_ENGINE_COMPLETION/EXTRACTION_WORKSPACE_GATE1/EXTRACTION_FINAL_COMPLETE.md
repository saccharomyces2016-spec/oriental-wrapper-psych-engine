...
**文件狀態**：COMPLETE（提取階段完成，待顧問團隊整合刪減昇華）
...
| **Narratives** | 13 | 13 | 0 | ✅ 100% | 已完成（部分有內容，部分待填充） |
| **Riskchains** | 13 | 13 | 0 | ✅ 100% | 已完成（部分有內容，部分待填充） |
...
- `raw/p1_question_blueprint_v1.json` - 2 個主題（已標註缺失）
...
- `contentdb/ContentDB_*.js` - 9 個主題（已標註來源，待手動分析提取）
...
|---------------|---------|-----------|---------|------------|----------------|------------|----------|
| `chronic_depletion` | 長期耗竭 | ✅ | ✅ | ✅（20 個錨定語句 + 3 個 ContentDB） | ✅ | ✅（1 個後果鏈 + chains.json） | ✅ |
| `hyper_responsibility` | 過度責任 | ✅ | ✅ | ✅（10 個錨定語句 + 2 個 ContentDB） | ✅ | ✅（chains.json） | ✅ |
| `fear_based_stability` | 恐懼驅動的穩定 | ✅ | ✅ | ✅（10 個錨定語句） | ✅ | ✅（1 個後果鏈 + chains.json） | ✅ |
| `loss_of_agency` | 主控感流失 | ✅ | ✅ | ⏳（4 個 ContentDB） | ✅ | ✅（chains.json） | ✅ |
| `social_comparison` | 社會比較壓力 | ✅ | ✅ | ✅（10 個錨定語句 + 3 個 ContentDB） | ⚠️ 缺失 | ✅（1 個後果鏈 + chains.json） | ✅ |
| `suppressed_needs` | 需求壓抑 | ✅ | ✅ | ⏳（2 個 ContentDB） | ✅ | ⏳ | ✅ |
| `identity_diffusion` | 自我模糊 | ✅ | ✅ | ✅（10 個錨定語句 + 1 個 ContentDB） | ✅ | ⏳ | ✅ |
| `chronic_alertness` | 長期警戒 | ✅ | ✅ | ⏳（1 個 ContentDB） | ✅ | ✅（1 個後果鏈） | ✅ |
| `meaning_vacuum` | 意義真空 | ✅ | ✅ | ⏳（1 個 ContentDB） | ✅ | ⏳ | ✅ |
| `self_erosion` | 自我耗損 | ⚠️ 缺失 | ✅ | ⏳ | ✅ | ⏳ | ✅ |
| `emotional_permeability` | 情緒滲透 | ✅ | ⏳ | ✅（10 個錨定語句 + 3 個 ContentDB） | ⚠️ 缺失 | ✅（1 個後果鏈 + chains.json） | ✅ |
| `avoidance_coping` | 逃避型因應 | ✅ | ⏳ | ✅（10 個錨定語句） | ⚠️ 缺失 | ✅（1 個後果鏈 + chains.json） | ✅ |
| `unprocessed_loss` | 未處理的失落 | ⚠️ 缺失 | ⏳ | ⏳ | ✅ | ⏳ | ✅ |
...
### 4.2 consequence_chain_library 提取統計
...
### 4.3 chains.json 提取統計

**已更新 riskchains**：
- `chronic_depletion`：burnout_overload
- `hyper_responsibility`：burnout_overload
- `fear_based_stability`：control_need, avoidance_procrastination, commitment_anxiety
- `loss_of_agency`：control_need, commitment_anxiety
- `social_comparison`：attachment_insecurity
- `emotional_permeability`：attachment_insecurity
- `avoidance_coping`：avoidance_procrastination

**總計**：10 個主題已從 chains.json 更新

---

### 4.4 ContentDB 來源標註統計

**已標註 ContentDB 來源**：
- `chronic_depletion`：3 個來源（self, health, career）
- `hyper_responsibility`：2 個來源（self, family）
- `loss_of_agency`：4 個來源（career, money, finance, study）
- `social_comparison`：3 個來源（relationship, social, love）
- `suppressed_needs`：2 個來源（self, family）
- `identity_diffusion`：1 個來源（self）
- `chronic_alertness`：1 個來源（health）
- `meaning_vacuum`：1 個來源（study）
- `emotional_permeability`：3 個來源（relationship, social, love）

**總計**：9 個主題已標註 ContentDB 來源

---

## 五、已知問題和缺失

### 5.1 缺失的 Questions（2 個主題）
...
│   ├── self_erosion.questions.v1.0.json（⚠️ 缺失）
│   ├── unprocessed_loss.questions.v1.0.json（⚠️ 缺失）
│   └── MISSING_QUESTIONS_NOTES.md
├── scoring/                # 計分資料（待提取，部分在 domain/）
├── narratives/             # 敘事資料（✅ 13/13，部分有內容）
│   ├── chronic_depletion.narr.v1.0.json（20 個錨定語句 + 3 個 ContentDB）
│   ├── hyper_responsibility.narr.v1.0.json（10 個錨定語句 + 2 個 ContentDB）
│   ├── fear_based_stability.narr.v1.0.json（10 個錨定語句）
│   ├── social_comparison.narr.v1.0.json（10 個錨定語句 + 3 個 ContentDB）
│   ├── identity_diffusion.narr.v1.0.json（10 個錨定語句 + 1 個 ContentDB）
│   ├── emotional_permeability.narr.v1.0.json（10 個錨定語句 + 3 個 ContentDB）
│   ├── avoidance_coping.narr.v1.0.json（10 個錨定語句）
│   └── ...（6 個主題待填充內容，已標註 ContentDB 來源）
...
│   ├── chronic_depletion.risk.v1.0.json（1 個後果鏈 + chains.json）
│   ├── hyper_responsibility.risk.v1.0.json（chains.json）
│   ├── fear_based_stability.risk.v1.0.json（1 個後果鏈 + chains.json）
│   ├── loss_of_agency.risk.v1.0.json（chains.json）
│   ├── social_comparison.risk.v1.0.json（1 個後果鏈 + chains.json）
│   ├── chronic_alertness.risk.v1.0.json（1 個後果鏈）
│   ├── emotional_permeability.risk.v1.0.json（1 個後果鏈 + chains.json）
│   ├── avoidance_coping.risk.v1.0.json（1 個後果鏈 + chains.json）
│   └── ...（5 個主題待填充內容）
...
├── EXTRACTION_COMPLETE_FINAL.md  # 完整提取最終報告
├── EXTRACTION_FINAL_COMPLETE.md  # 完整提取最終完成報告（本文件）
...
1. **補充缺失的 Questions**（2 個主題）
   - `self_erosion`、`unprocessed_loss`
   - 需要實際題目內容或標註缺失

2. **填充 Narratives**（6 個主題）
   - 從已標註的 ContentDB 來源提取 narratives
   - 或從其他來源提取

3. **填充 Riskchains**（5 個主題）
   - 從 buildGuidance.js 提取風險鏈生成邏輯
   - 或從其他來源提取

4. **補充缺失的 Recommendations**（3 個主題）
...
1. **已提取的內容**：
   ...
   - 所有題庫資料
   - 13 個主題的 manifests
   - 11 個主題的 questions（有實際內容）
   - 10 個主題的 recommendations（有實際內容）
   - 7 個主題的 narratives（有部分內容）
   - 10 個主題的 riskchains（有部分內容）

2. **已標註缺失的內容**：
   - 2 個主題的 questions（已標註缺失）
   - 3 個主題的 recommendations（已標註缺失）

3. **待填充的內容**：
   - 6 個主題的 narratives（已標註 ContentDB 來源）
   - 5 個主題的 riskchains（待從 buildGuidance.js 提取）
...
- ✅ Narratives（100%，13/13 個主題，部分有內容，部分已標註來源）
- ✅ Riskchains（100%，13/13 個主題，部分有內容，部分待填充）
...
2. **總整理報告**
   - `EXTRACTION_FINAL_COMPLETE.md` - 本文件
   - `EXTRACTION_FINAL_SUMMARY.md` - 總整理報告
   - `LEGACY_THEME_MAPPING.md` - Legacy 主題對應關係表
   - `EXTRACTION_CONTENT_ANALYSIS.md` - 內容分析報告
