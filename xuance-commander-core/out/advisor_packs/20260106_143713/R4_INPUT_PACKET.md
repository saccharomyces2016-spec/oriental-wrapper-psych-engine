...
---
# ROLE SECTION: R4 (Risk Chain)

## Deliverable
- Provide >=2 risk-chain frameworks (A->B->C)
- Mark earliest breakpoints and observable signals
- No fearmongering, no medicalization

## Forbidden
- Threatening language
- Medical/clinical framing
- Substituting facetId or redefining the domain


---
## FILE: docs/gem/briefs/P0-2 MASTER PROPOSAL
...
---
## FILE: docs/gem/briefs/P0-2 R4 riskchains brief
(path: /Users/yujunwei/Projects/115.1.4 oriental-wrapper-psych-engine/xuance-commander-core/docs/gem/briefs/BRIEF_P0-2_income_expansion_pressure_R4_riskchains.zh.md)

# BRIEF（P0-2｜income_expansion_pressure｜R4 風險鏈顧問）
generatedAt: 2026-01-05T18:20:32+0800

## 任務
請為 income_expansion_pressure 設計風險鏈框架（risk chains）：
- 至少針對 mid/high bands
- 每條風險鏈必須是可理解因果序列：A -> B -> C
- 每條鏈要提供「最早打斷點」（early interrupt point）

## 硬性約束（不得違反）
- 玄學語境對外呈現（不得醫療化/不得心理診斷語氣）
- 不恐嚇、不過度承諾
...
## research 參照（必查但不得外露原文）
- /Users/yujunwei/Projects/115.1.4 oriental-wrapper-psych-engine/xuance-commander-core/docs/research/

## 你要輸出的格式（固定）
1) bands 假設（你假設的 3~5 bands，用於風險鏈分層）
2) 每 band：2~4 條 risk chain（A->B->C）
3) 每條 chain：early interrupt point（最早介入點）
4) CN/EN 語氣規則（怎麼講才像玄學警示，而不是恐嚇）
5) 驗收：讀者看完能說出「我目前在哪一步」與「我能先做哪一步」
