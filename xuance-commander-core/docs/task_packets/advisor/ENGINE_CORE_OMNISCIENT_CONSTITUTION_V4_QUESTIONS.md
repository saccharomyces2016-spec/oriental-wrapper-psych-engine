# ENGINE_CORE_OMNISCIENT_CONSTITUTION_V4 追問包

**建立日期**：2026-01-13  
**狀態**：OPEN  
**關聯文件**：`ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md`（GPT V4 處理後版本）  
...
本追問包針對 `ENGINE_CORE_OMNISCIENT_CONSTITUTION_FINAL.md`（GPT V4 處理後版本）中需要進一步確認的關鍵問題，提供完整的背景資料，供顧問團隊進行決策。
...
#### 當前實作狀態
- `engine/score_facet.py`：GPT V4 已提供完整實作（支援 weighted_sum 和 vector_state_v3）
- `engine/cascade_calculator.py`：GPT V4 已提供完整實作
- `engine/narrative_guard.py`：GPT V4 已提供完整實作
- `tests/test_v3_scoring.py`：GPT V4 已提供測試
- `domain/facets/*.scoring.v1.0.json`：GPT V4 已提供兩個新 Facet 範例（burnout_syndrome, deep_depression）

#### 當前 Schema 結構
- `schemas/domain_manifest.schema.json`：GPT V4 已更新（新增 domainKey, questionSet, scoringModel）
- `schemas/compiled_facet.schema.json`：GPT V4 已更新（新增 domainKey, questionSet）
- `domain/domains/bagua.domain_map.v1.0.json`：GPT V4 已提供 Domain 配置文件範例
- ⚠️ **缺少**：`schemas/domain.schema.json`（獨立的 Domain Schema）
...
**DIRECTIVE REV.B Section 1.2（實際內容）**：
```python
if not profile:
...
**GPT V4 聲稱**：
- 最終裁示為 0.50（MIXED default），與 REV.B 一致

**實際檢查結果**：
- ⚠️ **GPT V4 的說法與 DIRECTIVE REV.B 的實際內容不一致**
- DIRECTIVE REV.B 的實際內容明確寫的是 `return 0.0`（不是 0.50）

**衝突**：三個地方對缺失 Priors 的 Rigidity 預設值不一致，且 GPT V4 的說法與 DIRECTIVE REV.B 的實際內容不一致。
...
- ✅ 明確寫入 Volatility 標準差模式為 sample stddev（CONSTITUTION Section 3.2）

**未完成的項目**：
- ⚠️ Domain Schema 定義（只有 JSON 範例，沒有 Schema 定義）
- ⚠️ ADR_0005 標準差模式更新（需要確認是否已更新）
- ⚠️ 世界級增強建議的詳細實作規格（只有範例，沒有詳細規格）
...
1. **缺失 Priors 時，Rigidity 的預設值應該是 0 還是 0.50？**
   - 選項 A：採用 0.0（符合 DIRECTIVE REV.B 的實際內容）
     - **理由**：DIRECTIVE REV.B 是技術裁示文件，具有更高的權威性；缺失 Priors 時，不應該假設使用者的歸因模式；0.0 是最安全的預設值（避免誤判）；GPT V4 的說法與 DIRECTIVE REV.B 的實際內容不一致
...
**相關文件**：
- `docs/directives/DIRECTIVE-2026-01-12-02-REV.B.md` Section 1.2（實際內容：`return 0.0`）
...
- GPT V4 聲稱「已完成」，但實際上只有 JSON 範例（`domain/domains/bagua.domain_map.v1.0.json`），沒有 Schema 定義（`schemas/domain.schema.json`）
